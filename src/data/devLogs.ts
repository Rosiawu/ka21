...
export const staticDevLogs: DevLogEntry[] = [
  {
...
export function getHighestDevLogVersion(entries: DevLogEntry[] = []): number {
  return [...staticDevLogs, ...entries].reduce((max, entry) => {
    return Math.max(max, parseVersionNumber(entry.version));
  }, 0);
}
